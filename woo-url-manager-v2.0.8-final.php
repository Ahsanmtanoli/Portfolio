...
class Woo_URL_Manager {
    ...
    public function add_custom_rewrite_rules() {
        ...
        try {
            ...
            // Add combined rewrite rules for both products and categories
            foreach ($settings['product_slugs'] as $slug) {
                if (empty($slug)) continue;
                
                // Combined rule for both products and categories: /watches/something/
                // This will be handled by support_custom_product_url to determine if it's a product or category
                add_rewrite_rule(
                    '^' . preg_quote($slug, '/') . '/([^/]+)/?$',
    ...
    public function custom_term_links($url, $term, $taxonomy) {
        try {
            ...
            // Handle product categories with custom URLs like /watches/men/
            if ($taxonomy === 'product_cat') {
                // Check if this category should use a custom parent slug
                $category_slug = $term->slug;
                
                // For main categories (watches, wall-clocks, accessories), use the slug as parent
                if (in_array($category_slug, $settings['product_slugs'])) {
                    return home_url(user_trailingslashit($category_slug));
                }
                
                // For sub-categories, try to find the appropriate parent
                // Check if the category has a parent that matches our slugs
                if ($term->parent > 0) {
                    $parent_term = get_term($term->parent, 'product_cat');
                    if ($parent_term && !is_wp_error($parent_term) && in_array($parent_term->slug, $settings['product_slugs'])) {
                        return home_url(user_trailingslashit($parent_term->slug . '/' . $category_slug));
                    }
                }
                
                // Default fallback: use "watches" as the parent for uncategorized product categories
                return home_url(user_trailingslashit($this->default_slug . '/' . $category_slug));
            }
    ...
    public function support_custom_product_url($vars) {
        try {
            ...
            // Handle URLs like /watches/something/ where 'something' could be a category or product
            if (isset($vars['woo_item_name']) && isset($vars['woo_url_slug'])) {
                $item_name = $vars['woo_item_name'];
                $parent_slug = $vars['woo_url_slug'];
                
                // Check if this is a valid parent slug
                if (in_array($parent_slug, $settings['product_slugs'])) {
                    
                    // First, check if this is a product category
                    $category = get_term_by('slug', $item_name, 'product_cat');
                    if ($category) {
                        // This is a category URL like /watches/men/
                        ...
                        error_log('Woo URL Manager: Successfully mapped category URL - parent: ' . $parent_slug . ', category: ' . $item_name);
                        return $vars;
                    }
                    ...
                    if ($post) {
                        // This is a product URL like /watches/product-name/
                        ...
                        error_log('Woo URL Manager: Successfully mapped product URL - parent: ' . $parent_slug . ', product: ' . $item_name);
                        return $vars;
                    }
            ...
            // Legacy support for old query format
            if (isset($vars['name']) && !isset($vars['post_type']) && !isset($vars['woo_item_name'])) {
                ...
                if (isset($vars['woo_url_slug']) && in_array($vars['woo_url_slug'], $settings['product_slugs'])) {
                    ...
                    if ($post) {
                        ...
                        error_log('Woo URL Manager: Successfully mapped legacy product URL - slug: ' . $vars['woo_url_slug'] . ', product: ' . $product_slug);
                    } else {
                        error_log('Woo URL Manager: Legacy product not found for slug "' . $product_slug . '" under "' . $vars['woo_url_slug'] . '"');
                    }
                }
            }
    ...
    public function debug_parse_request($wp) {
        try {
            if (isset($wp->query_vars['woo_item_name']) && isset($wp->query_vars['woo_url_slug'])) {
                error_log('Woo URL Manager: Parsed URL - parent: ' . $wp->query_vars['woo_url_slug'] . ', item: ' . $wp->query_vars['woo_item_name'] . ', post_type: ' . ($wp->query_vars['post_type'] ?? 'none'));
            }
            if (isset($wp->query_vars['name']) && isset($wp->query_vars['woo_url_slug'])) {
                error_log('Woo URL Manager: Parsed URL (legacy) - slug: ' . $wp->query_vars['woo_url_slug'] . ', name: ' . $wp->query_vars['name'] . ', post_type: ' . ($wp->query_vars['post_type'] ?? 'none'));
            }
            if (isset($wp->query_vars['product_cat'])) {
                error_log('Woo URL Manager: Parsed category URL - category: ' . $wp->query_vars['product_cat']);
            }
        } catch (Exception $e) {
    ...
    public function debug_query($wp) {
        try {
            if (is_404()) {
                ...
                if (isset($wp->query_vars['woo_item_name']) && isset($wp->query_vars['woo_url_slug'])) {
                    $debug_info[] = 'parent: ' . $wp->query_vars['woo_url_slug'] . ', item: ' . $wp->query_vars['woo_item_name'];
                }
                
                if (isset($wp->query_vars['name']) && isset($wp->query_vars['woo_url_slug'])) {
                    $debug_info[] = 'legacy - slug: ' . $wp->query_vars['woo_url_slug'] . ', name: ' . $wp->query_vars['name'];
                }
                
                if (isset($wp->query_vars['product_cat'])) {
                    $debug_info[] = 'category: ' . $wp->query_vars['product_cat'];
                }
                
                if (!empty($debug_info)) {
                    error_log('Woo URL Manager: 404 detected for URL - ' . implode(', ', $debug_info));
    ...
    public function render_settings_page() {
        ...
        <div class="wrap" style="font-family: sans-serif;">
            ...
            <form method="post" action="options.php">
                ...
                <p class="description">Edit paths for existing taxonomies registered via CPT UI. Click the delete button to remove a taxonomy.</p>
                <table class="form-table" id="taxonomy-table">
                    <?php foreach ($settings['taxonomies'] as $tax => $path): ?>
                    <tr id="taxonomy-row-<?php echo esc_attr($tax); ?>">
                        ...
                        <td>
                            ...
                            <button type="button" class="button button-secondary delete-taxonomy" data-taxonomy="<?php echo esc_attr($tax); ?>" style="margin-left: 10px; background: #dc3232; color: white; border-color: #dc3232;">
                                Delete
                            </button>
                        </td>
                    </tr>
                ...
                <!-- Hidden fields for taxonomies to delete -->
                <div id="delete-taxonomy-fields"></div>
            ...
            <div style="background: rgb(192,192,192); padding: 15px; margin-top: 20px; border-radius: 5px;">
                ...
                <ul style="color: rgb(46,46,46);">
                    ...
                    <li>Products without a category default to the 'watches' slug.</li>
                    <li>Product categories use custom URLs: e.g., /watches/men/, /wall-clocks/vintage/</li>
            ...
            jQuery(document).ready(function($) {
                var deletedTaxonomies = [];
                
                // Handle delete button clicks
                $(document).on('click', '.delete-taxonomy', function(e) {
                    e.preventDefault();
                    ...
                    if (confirm(confirmMessage)) {
                        // Add to deleted list
                        deletedTaxonomies.push(taxonomy);
                        ...
                        $('#delete-taxonomy-fields').append(
                            '<input type="hidden" name="<?php echo $this->option_key; ?>[delete_taxonomies][]" value="' + taxonomy + '">'
                        );
                        
                        // Show success message
                        var successMsg = $('<div class="notice notice-warning is-dismissible" style="margin: 10px 0;"><p>Taxonomy "' + taxonomy + '" will be deleted when you save the settings.</p></div>');
                        $('#taxonomy-table').after(successMsg);
                ...
                }).on('mouseleave', '.delete-taxonomy', function() {
                    $(this).css({
                        'background': '#dc3232',
                        'border-color': '#dc3232'
                    });
                ...
                // Warning before form submission if taxonomies are being deleted
                $('form').on('submit', function() {
                    if (deletedTaxonomies.length > 0) {
                        var message = 'You are about to delete ' + deletedTaxonomies.length + ' taxonomy/taxonomies: ' + deletedTaxonomies.join(', ') + '. Continue?';
